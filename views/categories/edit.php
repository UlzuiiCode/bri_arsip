...
<div class="max-w-2xl animate-fade-in">
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-0.5">Ubah nama atau deskripsi kategori</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden p-6">
        <form action="<?= BASE_URL ?>/index.php?page=categories.update&id=<?= $category['id'] ?>" method="POST">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">

            <div class="space-y-5">
                <div>
                    <label for="nama" class="mb-1.5 block text-sm font-medium text-slate-700">Nama Kategori</label>
                    <input type="text" name="nama" id="nama" required value="<?= htmlspecialchars($category['nama']) ?>"
                ...
                <div>
                    ...
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                              class="w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm outline-none transition-all focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"><?= htmlspecialchars($category['deskripsi'] ?? '') ?></textarea>
                </div>
            </div>
